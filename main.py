...
def _prompt_algorithm(default="dqn"):
    raw_value = input("\nSelect algorithm [dqn/pg] (default dqn): ").strip().lower()
    if not raw_value:
        return default

    if raw_value in {"dqn", "pg"}:
        return raw_value
...
#*********************************************************************************#
def _plot_combined_reward_history(
    step_rewards,
    per_agent_rewards,
    loss_history,
    metric_history=None,
...
#*********************************************************************************#
def _export_training_history_csv(step_rewards, per_agent_rewards, loss_history, output_path="training_history.csv"):
...
#*********************************************************************************#
def run_DQN(num_episodes=100, env = None, max_steps_per_episode=2000, epsilon = 1.0, epsilon_decay=0.9995, epsilon_min=0.1, render=True, render_sleep=0.02, verbose=False, load_saved_model=False):
    ...
    try:
        for episode in range(1, num_episodes + 1):
            if not verbose:
                progress_bar(episode, num_episodes)
            observations, infos = env.reset()
...
#*********************************************************************************#
def run_PG(num_episodes=100, env = None, max_steps_per_episode=2000, render=True, render_sleep=0.02, verbose=False, load_saved_model=False):
